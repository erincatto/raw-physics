...
static Light* lights;
static r64 thrown_objects_initial_linear_velocity_norm = 30.0;
	...
	for (u32 i = 0; i < N; ++i) {
		...
		for (u32 j = 0; j < N; ++j) {
			...
			for (u32 k = 0; k < N; ++k) {
				...
				entity_create(cube_mesh, {x, y, z}, quaternion_new({0.0, 1.0, 0.0}, 0.0),
					cube_scale, util_pallete(i + j + k), 1.0, cube_colliders, 0.8, 0.8, 0.0);
			}
		}
	}
...
Example_Scene cube_storm_example_scene = {
	.name = "Cube Storm",

	...
	*indices = array_new(u32);
	...
	if (generate_normals) {
		...
		for (size_t i = 0; i < indices_length; i += 3) {
			...
			fvec3 A = {vertex_a->position.x, vertex_a->position.y, vertex_a->position.z};
			fvec3 B = {vertex_b->position.x, vertex_b->position.y, vertex_b->position.z};
			fvec3 C = {vertex_c->position.x, vertex_c->position.y, vertex_c->position.z};
			fvec3 first_edge = {B.x - A.x, B.y - A.y, B.z - A.z};
			fvec3 second_edge = {C.x - A.x, C.y - A.y, C.z - A.z};
			fvec4 normal;
			...
			// Assign normals
			vertex_a->normal = {vertex_a->normal.x + normal.x, vertex_a->normal.y + normal.y, vertex_a->normal.z + normal.z};
			vertex_b->normal = {vertex_b->normal.x + normal.x, vertex_b->normal.y + normal.y, vertex_b->normal.z + normal.z};
			vertex_c->normal = {vertex_c->normal.x + normal.x, vertex_c->normal.y + normal.y, vertex_c->normal.z + normal.z};
		}
	}

...
static Perspective_Camera create_camera() {
	...
	camera.rotation = {0.134854, 0.000000, 0.000000, 0.990883};
	camera.yrotation = {0.000000, 0.262195, 0.000000, 0.965035};
	camera.position = {-6.386153, 7.327643, 20.298402};
...
int ex_brick_wall_init() {
	...
	for (u32 i = 0; i < 6; ++i) {
		...
		for (u32 j = 0; j < 4; ++j) {
			vec3 cube_scale = {brick_width, brick_height, brick_height};
			...
			eid brick_eid = entity_create(cube_mesh, {x, y, 0.0}, quaternion_new({0.0, 1.0, 0.0}, 0.0),
...
void ex_brick_wall_update(r64 delta_time) {
	...
	pbd_simulate(delta_time, entities, subStepCount, posIter, true);
...
Example_Scene brick_wall_example_scene = {
	.name = "Brick Wall",
